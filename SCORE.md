...
From 2026-08-01 to 2026-08-02 `SCORE.tsv` also held a `checkpoint` row per
scoring run, and a `wall_s` column that only those rows filled. Both are gone. A
scoring run measures the working tree, so 29 of those 37 rows described an
uncommitted tree while naming HEAD, and none of them stated a figure an event
row did not already carry. They remain in `SCORE.tsv`'s Git history.
