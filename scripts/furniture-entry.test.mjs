...
// reach a second hero destination.
const SEGMENT_KEYS = new Set(['h', 'k', 's', RUN_NORTH, RUSH_NORTH]);
...
test('every matrix segment replays to its last key', async () => {
    ...
    // segment that stops early emits fewer. With both destination predicates
    // narrowed back to STAIRS, ten of these eleven segments stopped short; the
    // Ctrl-rush segment that stops in front of its fountain is the exception.
