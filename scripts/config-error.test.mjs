...
const COMPOUND_SWEEP_REPORTS = new Map([
    ['autounlock', [
        [],
        [],
        ['Invalid value for "autounlock": "zqxj".'],
    ]],
    ['playmode', [
        [],
        [],
    ...
    ['crash_email', [
        ["Missing parameter for 'crash_email'."],
        ["Missing parameter for 'crash_email:'."],
        [],
    ]],
    ['crash_name', [
    ...
    ['soundlib', [
        ["Missing parameter for 'soundlib'."],
        ["Missing parameter for 'soundlib:'."],
        [],
    ]],
    ['tile_height', [
    ...
    ['player_selection', [
        ["Missing parameter for 'player_selection'."],
        ["Missing parameter for 'player_selection:'."],
        ["Unknown player_selection parameter 'zqxj'."],
    ]],
    ['sortloot', [
    ...
    ['whatis_filter', [
        ...
        ["Missing parameter for 'whatis_filter:'."],
        ["Unknown whatis_filter parameter 'zqxj'."],
    ]],
]);
...
test('every compound option reports exactly what this parser owes it', () => {
    ...
    assert.equal(COMPOUND_SWEEP_REPORTS.size, 68);
    assert.equal(SILENT_COMPOUND_ROWS.size, 17);
    assert.equal(UNPORTED_COMPOUND_ROWS.size, 10);
    ...
    // 27 messages over 10 rows: what porting those handlers is worth to a
    // configuration file that names one.
    assert.equal(owed, 27);
});
