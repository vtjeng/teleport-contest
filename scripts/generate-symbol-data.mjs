...
export function extractSymbolLayout(source) {
    ...
    // drawing.c def_monsyms[].explain, whose entry 0 is the "random class"
    // placeholder `{ '\0', "", "" }` that defsym.h does not describe.
    const monsterClassExplanations = new Array(MAXMCLASSES).fill('');
    ...
    const monsterClassEntry =
        /^\s*MONSYM\(\s*(\d+)\s*,\s*'(?:\\.|[^'])+'\s*,([\s\S]*?)\)$/gmu;
    for (const entry of source.matchAll(monsterClassEntry)) {
        const quoted = [...entry[2].matchAll(/"((?:\\.|[^"])*)"/gu)];
        if (!quoted.length)
            throw new Error(`MONSYM entry ${entry[1]} has no explanation`);
        monsterClassExplanations[Number(entry[1])] = quoted.at(-1)[1];
    }
...
export function renderSymbolData(definitions, layout, cmapColors) {
    ...
    const monsterClassExplanations = JSON.stringify(
    ...
    return `// Generated by scripts/generate-symbol-data.mjs.\n`
        ...
        + `export const MONSTER_CLASS_EXPLANATIONS = `
        + `Object.freeze(${monsterClassExplanations});\n\n`
