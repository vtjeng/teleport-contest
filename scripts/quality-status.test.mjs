...
test('the ledger declares the six commits that sit behind a frontier', async () => {
  const config = JSON.parse(
    await readFile(new URL('../QUALITY.json', import.meta.url), 'utf8'),
  );
  const declared = Object.fromEntries(
    config.areas
      .filter((area) => area.unreviewedCommits)
      .map((area) => [area.id, area.unreviewedCommits.length]),
  );

  // The set established by hand in ROADMAP.md, "unreviewed commits behind a
  // review frontier". It is closed: validateAuditedRangeCoverage refuses a
  // base after the frontier, so no later pass can add to it.
  assert.deepEqual(declared, { startup: 2, hero: 2, monsters: 1, commands: 1 });
});
...
test('declared unreviewed commits must be full SHAs owned by one area', () => {
  ...
  const area = (id, extra) => ({ id, label: id, paths: [`js/${id}.js`], ...extra });

  assert.doesNotThrow(() => validateConfigShape({
    ...base,
    areas: [area('one', { unreviewedCommits: [sha] })],
  }));
  // A short SHA cannot be resolved later without ambiguity.
  assert.throws(
    () => validateConfigShape({
      ...base,
      areas: [area('one', { unreviewedCommits: ['c'.repeat(7)] })],
    }),
    /not a full SHA/,
  );
  // One commit, one owning area: two claims would double-count the debt.
  assert.throws(
    () => validateConfigShape({
      ...base,
      areas: [
        area('one', { unreviewedCommits: [sha] }),
        area('two', { unreviewedCommits: [sha] }),
      ],
    }),
    /listed unreviewed by both one and two/,
  );
  // An empty array claims debt exists while naming none of it.
  assert.throws(
    () => validateConfigShape({
      ...base,
      areas: [area('one', { unreviewedCommits: [] })],
    }),
