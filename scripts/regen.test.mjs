...
test('regen_hp heals on the level and Constitution sum alone', () => {
    const heals = (configure) => {
        const state = regenState();
        state.u.acurr.a[A_CON] = 15;
        configure?.(state);
        const bounds = [];
        const healed = regen_hp(0, state, {
            random: {
                rn2(bound) {
                    bounds.push(bound);
                    return state.draw;
    ...
    assert.equal(heals((state) => { state.draw = 15; }), true);
    assert.equal(heals((state) => { state.draw = 16; }), false);
    // A higher experience level raises the same threshold, so a heal that
    // ignored ulevel would pass the pair above and fail here.
    assert.equal(heals((state) => { state.u.ulevel = 3; state.draw = 17; }), true);
    assert.equal(heals((state) => { state.u.ulevel = 3; state.draw = 18; }), false);
    ...
    assert.equal(heals((state) => { state.u.abon[A_CON] = 1; state.draw = 16; }), true);
    assert.equal(heals((state) => { state.u.atemp[A_CON] = 1; state.draw = 16; }), true);
});

test('regen_hp adds each bonus and clamps at the maximum', () => {
    const state = regenState();
    state.u.uhpmax = 20;
    ...
    assert.equal(regen_hp(0, state, { random: { rn2: () => 0 } }), true);
...
test('regen_hp respects the encumbrance gate at its boundary', () => {
    ...
    assert.equal(regen_hp(MOD_ENCUMBER, moved, {
    ...
    const bounds = [];
    assert.equal(regen_hp(MOD_ENCUMBER, still, {
    ...
    assert.equal(regen_hp(MOD_ENCUMBER, regenerating, {
        random: { rn2: () => 99 },
    }), true);
...
test('regen_pw draws only on the role cadence', () => {
    ...
    for (const moves of [23, 25]) {
        ...
        assert.equal(regen_pw(0, offCadence, {
...
test('regen_pw draws off the cadence with Energy_regeneration', () => {
    ...
    without.moves = 23;
    assert.equal(regen_pw(0, without, {
...
test('regen_pw uses role cadence and magical-breathing upper bound', () => {
    ...
    clamped.moves = 24;
    clamped.u.uen = 4;
    assert.equal(regen_pw(0, clamped, {
        random: { rn1: () => 3 },
        interruptMulti: () => {},
    }), true);
