...
- Native Codex subagents are available to both the primary session and processes
  ...
  work that benefits from parallelism. Formal skill passes may also parallelize,
  but the pass itself must remain independent from the primary session. Launch
  each pass in a fresh top-level `codex exec --ephemeral` process, give it only
  the scoped review inputs, and do not reuse it for another pass. That process is
  the pass orchestrator and may assign the skill's reviewer roles to its own
  native subagents. Do not substitute a subagent spawned by the primary session
  for the fresh top-level process.
