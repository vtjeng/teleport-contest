...
async function medusa1(des) {
    ...
    // Dungeon Description
    await des.region(selection_area(0, 0, 74, 19), 'lit');
    await des.region(selection_area(31, 7, 45, 7), 'unlit');
    ...
    await des.region(selection_area(31, 12, 45, 12), 'unlit');
    ...
    // Stairs
    await des.stair({ dir: 'up', coord: [5, 14] });
    await des.stair({ dir: 'down', coord: [36, 10] });
    ...
    // Non diggable walls
    await des.non_diggable(selection_area(30, 6, 46, 13));
    ...
    await des.object({
        ...
        async contents() {
            if (percent(75)) {
                await des.object({ id: SHIELD_OF_REFLECTION, buc: 'cursed', spe: 0 });
            }
            if (percent(25)) {
                await des.object({ id: LEVITATION_BOOTS, spe: 0 });
            }
            if (percent(50)) {
    ...
    await des.trap();
    await des.trap();
    await des.trap({ type: 'board', coord: [38, 7] });
    await des.trap({ type: 'board', coord: [38, 12] });

    // Random monsters
    await des.monster({ id: PM_MEDUSA, x: 36, y: 10, asleep: 1 });
    await des.monster({ id: PM_GIANT_EEL, coord: [11, 6] });
    await des.monster({ id: PM_GIANT_EEL, coord: [23, 13] });
    await des.monster({ id: PM_GIANT_EEL, coord: [29, 2] });
    await des.monster({ id: PM_JELLYFISH, coord: [2, 2] });
    await des.monster({ id: PM_JELLYFISH, coord: [0, 8] });
    await des.monster({ id: PM_JELLYFISH, coord: [4, 18] });
    await des.monster({ id: PM_WATER_TROLL, coord: [51, 3] });
    await des.monster({ id: PM_WATER_TROLL, coord: [64, 11] });
    await des.monster({ class: S_SNAKE, x: 38, y: 7 });
    await des.monster({ class: S_SNAKE, x: 38, y: 12 });
    await des.monster();
    await des.monster();
    await des.monster();
    await des.monster();
...
async function medusa2(des) {
    ...
    // Dungeon Description
    await des.region(selection_area(0, 0, 74, 19), 'lit');
    await des.region(selection_area(2, 3, 5, 16), 'unlit');
    ...
    await des.region(selection_area(71, 8, 72, 11), 'unlit');
    // Downstairs area
    ...
    // Stairs
    await des.stair({ dir: 'up', coord: [4, 9] });
    await des.stair({ dir: 'down', coord: [68, 10] });
    ...
    // Non diggable walls
    await des.non_diggable(selection_area(1, 2, 6, 17));
    await des.non_diggable(selection_area(60, 2, 73, 17));
    ...
    await des.object({ id: STATUE, x: 64, y: 8, contents() {} });
    await des.object({ id: STATUE, x: 65, y: 8, contents() {} });
    await des.object({ id: STATUE, x: 64, y: 9, contents() {} });
    await des.object({ id: STATUE, x: 65, y: 9, contents() {} });
    await des.object({ id: STATUE, x: 64, y: 10, contents() {} });
    await des.object({ id: STATUE, x: 65, y: 10, contents() {} });
    await des.object({ id: STATUE, x: 64, y: 11, contents() {} });
    await des.object({ id: STATUE, x: 65, y: 11, contents() {} });

    // Named objects
    await des.object({ id: BOULDER, coord: [4, 4] });
    await des.object({ class: def_char_to_objclass('/'), coord: [52, 9] });
    await des.object({ id: BOULDER, coord: [52, 9] });
    ...
    // Traps
    await des.trap({ type: 'magic', coord: [3, 12] });
    await des.trap();
    await des.trap();
    await des.trap();
    await des.trap();
    ...
    // Monsters
    await des.monster({ id: PM_MEDUSA, x: 68, y: 10, asleep: 1 });
    await des.monster({ id: PM_GREMLIN, coord: [2, 14] });
    await des.monster({ id: PM_TITAN, coord: [2, 5] });
    await des.monster({ id: PM_ELECTRIC_EEL, coord: [10, 13] });
    await des.monster({ id: PM_ELECTRIC_EEL, coord: [11, 13] });
    await des.monster({ id: PM_ELECTRIC_EEL, coord: [10, 14] });
    await des.monster({ id: PM_ELECTRIC_EEL, coord: [11, 14] });
    await des.monster({ id: PM_ELECTRIC_EEL, coord: [10, 15] });
    await des.monster({ id: PM_ELECTRIC_EEL, coord: [11, 15] });
    await des.monster({ id: PM_JELLYFISH, coord: [1, 1] });
    await des.monster({ id: PM_JELLYFISH, coord: [0, 8] });
    await des.monster({ id: PM_JELLYFISH, coord: [4, 19] });
    await des.monster({ id: PM_STONE_GOLEM, x: 64, y: 8, asleep: 1 });
    await des.monster({ id: PM_STONE_GOLEM, x: 65, y: 8, asleep: 1 });
    await des.monster({ id: PM_STONE_GOLEM, x: 64, y: 9, asleep: 1 });
    await des.monster({ id: PM_STONE_GOLEM, x: 65, y: 9, asleep: 1 });
    await des.monster({ id: PM_COBRA, x: 64, y: 10, asleep: 1 });
    await des.monster({ id: PM_COBRA, x: 65, y: 10, asleep: 1 });
    await des.monster({ class: def_char_to_monclass('A'), coord: [72, 8] });
    await des.monster({ id: PM_YELLOW_LIGHT, x: 72, y: 11, asleep: 1 });
...
async function medusa3(des) {
    ...
    await des.region(selection_area(7, 5, 9, 7), 'unlit');
    await des.region(selection_area(65, 4, 67, 6), 'unlit');
    await des.region(selection_area(45, 14, 47, 16), 'unlit');

    // Non diggable walls
    await des.non_diggable(selection_area(6, 4, 10, 8));
    await des.non_diggable(selection_area(64, 3, 68, 7));
    await des.non_diggable(selection_area(44, 13, 48, 17));
    ...
    await des.feature('fountain', othloc.x, othloc.y);
    ...
    await des.object({ id: SCR_BLANK_PAPER, coord: [48, 18] });
    await des.object({ id: SCR_BLANK_PAPER, coord: [48, 18] });

    // Traps
    await des.trap({ type: 'rust' });
    await des.trap({ type: 'rust' });
    await des.trap({ type: 'board' });
    await des.trap({ type: 'board' });
    await des.trap();
    ...
    await des.monster({ id: PM_JELLYFISH });
    await des.monster({ id: PM_WOOD_NYMPH });
    await des.monster({ id: PM_WOOD_NYMPH });
    await des.monster({ id: PM_WATER_NYMPH });
    await des.monster({ id: PM_WATER_NYMPH });

    // 30 hostile ravens nesting in the trees
    for (let i = 0; i < 30; i++) {
        await des.monster({ id: PM_RAVEN, peaceful: 0 });
    }
}
