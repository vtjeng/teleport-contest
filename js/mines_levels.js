...
async function minefill(des) {
    ...
    await des.monster({ id: PM_GNOME_LEADER, parsedGender: MALE });
    await des.monster({ id: PM_DWARF });
    await des.monster({ id: PM_DWARF });
    await des.monster({ class: def_char_to_monclass('G') });
    await des.monster({ class: def_char_to_monclass('G') });
    await des.monster({
...
async function minetn1(des, state) {
    ...
    await des.region(selection_area(1, 1, 35, 17), 'lit');
    await des.levregion({
        type: 'stair-up',
        region: [1, 3, 21, 19],
        region_islev: 1,
    ...
    await des.object({ id: CORPSE, coord: [20, 12], montype: PM_ALIGNED_CLERIC });
    await des.object({ id: CORPSE, coord: place[0], montype: PM_SHOPKEEPER });
    await des.object({ id: CORPSE, coord: place[1], montype: PM_SHOPKEEPER });
    await des.object({ id: CORPSE, coord: place[2], montype: PM_SHOPKEEPER });
    await des.object({ id: CORPSE, coord: place[3], montype: PM_SHOPKEEPER });
    await des.object({ id: CORPSE, coord: place[4], montype: PM_SHOPKEEPER });
    await des.object({ id: CORPSE, montype: PM_WATCHMAN });
    await des.object({ id: CORPSE, montype: PM_WATCHMAN });
    await des.object({ id: CORPSE, montype: PM_WATCHMAN });
    await des.object({ id: CORPSE, montype: PM_WATCHMAN });
    await des.object({ id: CORPSE, montype: PM_WATCH_CAPTAIN });
    ...
    for (let i = 0; i < rubbleCount; i++) {
        ...
        await des.object({ id: ROCK });
    }

    // C ref: minetn-1.lua:98-105. Candles totaling at least 7.
    await des.object({ id: WAX_CANDLE, coord: place[3], quantity: rn2(2) + 1 });
    await des.object({ id: WAX_CANDLE, coord: place[0], quantity: rn2(3) + 2 });
    await des.object({ id: WAX_CANDLE, coord: place[1], quantity: rn2(2) + 1 });
    await des.object({ id: TALLOW_CANDLE, coord: place[2], quantity: rn2(3) + 1 });
    await des.object({ id: TALLOW_CANDLE, coord: place[1], quantity: rn2(2) + 1 });
    await des.object({ id: TALLOW_CANDLE, coord: place[3], quantity: rn2(2) + 1 });

    await des.object({ id: OIL_LAMP, coord: place[1] });
    await des.object({
    ...
    for (let i = 0; i < outsideCount; i++) {
        if (percent(90)) {
            await des.monster({ id: PM_HILL_ORC, peaceful: 0 });
        } else {
            await des.monster({ id: PM_GOBLIN, peaceful: 0 });
        }
    }
        ...
        async contents() {
            ...
            await des.room({
                ...
                async contents() {
                    ...
                    await des.monster({ id: PM_GNOMISH_WIZARD });
                    await des.monster({ id: PM_GNOMISH_WIZARD });
                },
            });
            ...
            await des.monster({ id: PM_WATCHMAN, peaceful: 1 });
            await des.monster({ id: PM_WATCHMAN, peaceful: 1 });
            await des.monster({ id: PM_WATCHMAN, peaceful: 1 });
            await des.monster({ id: PM_WATCHMAN, peaceful: 1 });
            await des.monster({ id: PM_WATCH_CAPTAIN, peaceful: 1 });
        },
    });
...
async function minetn3(des, state) {
    ...
    await des.room({
        type: 'ordinary',
        async contents() { await des.stair('up'); },
    });
    await des.room({
        type: 'ordinary',
        async contents() {
    ...
    await des.room({
        type: 'ordinary',
        async contents() { await des.monster({ id: PM_DWARF }); },
    });
    await des.room({
        type: 'ordinary',
        async contents() {
        ...
        async contents() {
            ...
            await des.room({
                ...
                async contents() {
                    ...
                    await des.monster({ id: PM_KOBOLD_SHAMAN });
                    await des.monster({ id: PM_KOBOLD_SHAMAN });
                    await des.monster({ id: PM_KITTEN });
                    await des.monster({ class: def_char_to_monclass('f') });
                },
            });
            await des.room({
                type: monkfoodshop(state), chance: 90, lit: 1,
...
async function minetn5(des, state) {
    ...
    if (percent(75)) {
        if (percent(50)) {
            await des.terrain(selection_area(25, 8, 25, 9), '|');
        } else {
            await des.terrain(selection_area(16, 13, 17, 13), '-');
        }
    }
    if (percent(75)) {
        if (percent(50)) {
            await des.terrain(selection_area(36, 10, 36, 11), '|');
        } else {
            await des.terrain(selection_area(32, 15, 33, 15), '-');
        }
    }
    if (percent(50)) {
        await des.terrain(selection_area(21, 4, 22, 5), '.');
        await des.terrain(selection_area(14, 9, 14, 10), '|');
    }
    if (percent(50)) {
        await des.terrain([46, 13], '|');
        await des.terrain(selection_area(43, 5, 47, 5), '-');
        await des.terrain(selection_area(42, 6, 46, 6), '.');
        await des.terrain(selection_area(46, 7, 47, 7), '.');
    }
    if (percent(50)) {
        await des.terrain(selection_area(69, 11, 71, 11), '-');
    }

    await des.stair({ dir: 'up', coord: [1, 1] });
    await des.stair({ dir: 'down', coord: [46, 3] });
    await des.feature('fountain', 50, 9);
    await des.feature('fountain', 10, 15);
    await des.feature('fountain', 66, 18);

    await des.region(selection_area(0, 0, 74, 20), 'unlit');
    await des.region(selection_area(9, 13, 11, 17), 'lit');
    await des.region(selection_area(8, 14, 12, 16), 'lit');
    await des.region(selection_area(49, 7, 51, 11), 'lit');
    await des.region(selection_area(48, 8, 52, 10), 'lit');
    await des.region(selection_area(64, 17, 68, 19), 'lit');
    await des.region(selection_area(37, 13, 39, 17), 'lit');
    await des.region(selection_area(36, 14, 40, 17), 'lit');
    await des.region(selection_area(59, 2, 72, 10), 'lit');
    ...
    await des.monster({ id: PM_GNOMISH_WIZARD, coord: [2, 19] });
    await des.door('locked', 20, 16);
    await des.monster({ class: def_char_to_monclass('G'), coord: [20, 18] });
    await des.door('random', 21, 14);
    await des.door('random', 25, 14);
    await des.door('random', 42, 8);
    await des.door('locked', 40, 5);
    await des.monster({ class: def_char_to_monclass('G'), coord: [38, 7] });
    ...
    await des.monster({ id: PM_GNOMISH_WIZARD, coord: [67, 14] });
    await des.object({ class: RING_CLASS, coord: [70, 14] });
    await des.door('locked', 69, 18);
    await des.monster({
        id: PM_GNOME_LEADER, parsedGender: MALE, coord: [71, 19],
    });
    await des.door('locked', 73, 18);
    await des.object({ id: CHEST, coord: [73, 19] });
    await des.door('locked', 50, 6);
    await des.object({ class: TOOL_CLASS, coord: [50, 3] });
    await des.object({
...
async function minetn6(des, state) {
    await des.level_init({ style: 'solidfill', fg: ' ' });
    await des.level_flags('mazelevel', 'inaccessibles');
    ...
    await des.region(selection_area(0, 0, 39, 19), 'lit');
    ...
    await des.region(selection_area(13, 7, 14, 8), 'unlit');
    await des.region({
    ...
    await des.monster({ id: PM_GNOME });
    await des.monster({ id: PM_GNOME });
    await des.monster({ id: PM_GNOME, coord: [14, 8] });
    await des.monster({
        id: PM_GNOME_LEADER, parsedGender: MALE, coord: [14, 7],
    });
    await des.monster({ id: PM_GNOME, coord: [27, 10] });
    await des.monster({ id: PM_GNOME_LEADER, parsedGender: MALE });
    await des.monster({ id: PM_GNOME_LEADER, parsedGender: MALE });
    await des.monster({ id: PM_DWARF });
    await des.monster({ id: PM_DWARF });
    await des.monster({ id: PM_DWARF });
    await des.monster({ id: PM_DWARF, peaceful: 1 });
    await des.monster({ id: PM_DWARF, peaceful: 1 });
    await des.monster({ id: PM_GNOME, peaceful: 1 });
    await des.monster({ id: PM_GNOME, peaceful: 1 });
    await des.monster({ id: PM_HOBBIT, peaceful: 1 });
    await des.monster({ id: PM_GOBLIN, peaceful: 1 });
    await des.monster({ id: PM_KOBOLD, peaceful: 1 });
    await des.monster({ id: PM_DOG, peaceful: 1 });
    await des.monster({ id: PM_WATCHMAN, peaceful: 1 });
    await des.monster({ id: PM_WATCHMAN, peaceful: 1 });
    await des.monster({ id: PM_WATCHMAN, peaceful: 1 });
    await des.monster({ id: PM_WATCH_CAPTAIN, peaceful: 1 });
        ...
        async contents() {
            ...
            if (percent(75)) {
                await des.room({
                    ...
                    async contents() {
                        await des.monster({ id: PM_GNOME });
                        await des.monster({ id: PM_MONKEY });
                        await des.monster({ id: PM_MONKEY });
                        await des.monster({ id: PM_MONKEY });
                        await des.door({ state: 'closed', wall: 'south' });
                    },
...
async function minend1(des, state) {
    ...
    await des.region(selection_area(20, 8, 21, 8), 'unlit');
    await des.region(selection_area(23, 8, 25, 8), 'unlit');
    ...
    await des.stair({ dir: 'up', coord: [36, 4] });

    await des.non_diggable(selection_area(0, 0, 74, 17));

    // Niches — place[5] (Lua place[6]) is empty
    await des.object({ id: DIAMOND, coord: place[6] });
    await des.object({ id: EMERALD, coord: place[6] });
    await des.object({ id: WORTHLESS_VIOLET_GLASS, coord: place[6] });
    await des.monster({ class: def_char_to_monclass('m'), coord: place[6], appear_as: 'obj:luckstone' });
    await des.object({ id: WORTHLESS_WHITE_GLASS, coord: place[0] });
    await des.object({ id: EMERALD, coord: place[0] });
    await des.object({ id: AMETHYST, coord: place[0] });
    await des.monster({ class: def_char_to_monclass('m'), coord: place[0], appear_as: 'obj:loadstone' });
    await des.object({ id: DIAMOND, coord: place[1] });
    await des.object({ id: WORTHLESS_GREEN_GLASS, coord: place[1] });
    await des.object({ id: AMETHYST, coord: place[1] });
    await des.monster({ class: def_char_to_monclass('m'), coord: place[1], appear_as: 'obj:flint' });
    await des.object({ id: WORTHLESS_WHITE_GLASS, coord: place[2] });
    await des.object({ id: EMERALD, coord: place[2] });
    await des.object({ id: WORTHLESS_VIOLET_GLASS, coord: place[2] });
    await des.monster({ class: def_char_to_monclass('m'), coord: place[2], appear_as: 'obj:touchstone' });
    await des.object({ id: WORTHLESS_RED_GLASS, coord: place[3] });
    await des.object({ id: RUBY, coord: place[3] });
    await des.object({ id: LOADSTONE, coord: place[3] });
    await des.object({ id: RUBY, coord: place[4] });
    await des.object({ id: WORTHLESS_RED_GLASS, coord: place[4] });
    ...
    await des.monster({ id: PM_GNOME_RULER, parsedGender: MALE });
    await des.monster({ id: PM_GNOME_LEADER, parsedGender: MALE });
    await des.monster({ id: PM_GNOME_LEADER, parsedGender: MALE });
    await des.monster({ id: PM_GNOME_LEADER, parsedGender: MALE });
    await des.monster({ id: PM_GNOMISH_WIZARD });
    ...
    await des.monster({ id: PM_GNOME });
    await des.monster({ id: PM_GNOME });
    await des.monster({ id: PM_HOBBIT });
    await des.monster({ id: PM_HOBBIT });
    await des.monster({ id: PM_DWARF });
    await des.monster({ id: PM_DWARF });
    await des.monster({ id: PM_DWARF });
    await des.monster({ class: def_char_to_monclass('h') });
}
...
async function minend2(des, state) {
    ...
    if (percent(50)) {
        ...
        await des.terrain(selection_area(7, 12, 8, 13), '.');
    }
    if (percent(50)) {
    ...
    await des.region(selection_area(23, 3, 48, 6), 'lit');
    await des.region(selection_area(21, 6, 22, 6), 'lit');
    await des.region(selection_area(14, 4, 14, 4), 'unlit');
    await des.region(selection_area(10, 5, 14, 8), 'unlit');
    await des.region(selection_area(10, 9, 11, 9), 'unlit');
    await des.region(selection_area(15, 8, 16, 8), 'unlit');
    ...
    await des.non_diggable(selection_area(0, 0, 52, 17));
    await des.non_diggable(selection_area(53, 0, 74, 0));
    await des.non_diggable(selection_area(53, 17, 74, 17));
    await des.non_diggable(selection_area(74, 1, 74, 16));
    await des.non_diggable(selection_area(53, 7, 55, 7));
    await des.non_diggable(selection_area(53, 14, 61, 14));

    await des.engraving({ coord: [12, 3], type: 'engrave', text: 'You are now entering the Gnome King\'s wine cellar.' });
    await des.engraving({ coord: [12, 4], type: 'engrave', text: 'Trespassers will be persecuted!' });
    await des.object({ id: POT_BOOZE, coord: [10, 7] });
    await des.object({ id: POT_BOOZE, coord: [10, 7] });
    await des.object({ class: POTION_CLASS, coord: [10, 7] });
    await des.object({ id: POT_BOOZE, coord: [10, 8] });
    await des.object({ id: POT_BOOZE, coord: [10, 8] });
    await des.object({ class: POTION_CLASS, coord: [10, 8] });
    await des.object({ id: POT_BOOZE, coord: [10, 9] });
    await des.object({ id: POT_BOOZE, coord: [10, 9] });
    await des.object({ id: POT_OBJECT_DETECTION, coord: [10, 9] });

    await des.object({ id: DIAMOND, coord: [69, 4] });
    await des.object({ class: GEM_CLASS, coord: [69, 4] });
    await des.object({ id: DIAMOND, coord: [69, 4] });
    await des.object({ class: GEM_CLASS, coord: [69, 4] });
    await des.object({ id: EMERALD, coord: [70, 4] });
    await des.object({ class: GEM_CLASS, coord: [70, 4] });
    await des.object({ id: EMERALD, coord: [70, 4] });
    await des.object({ class: GEM_CLASS, coord: [70, 4] });
    await des.object({ id: EMERALD, coord: [69, 5] });
    await des.object({ class: GEM_CLASS, coord: [69, 5] });
    await des.object({ id: RUBY, coord: [69, 5] });
    await des.object({ class: GEM_CLASS, coord: [69, 5] });
    await des.object({ id: RUBY, coord: [70, 5] });
    await des.object({ id: AMETHYST, coord: [70, 5] });
    await des.object({ class: GEM_CLASS, coord: [70, 5] });
    await des.object({ id: AMETHYST, coord: [70, 5] });
    await des.object({ id: LUCKSTONE, coord: [70, 5], buc: 'not-cursed', achievement: 1 });
...
async function minend3(des, state) {
    ...
    await des.non_diggable(selection_area(67, 3, 73, 7));
    await des.non_diggable(selection_area(0, 12, 2, 16));
    await des.feature('fountain', [12, 8]);
    await des.feature('fountain', [51, 15]);
    await des.region(selection_area(0, 0, 75, 16), 'unlit');
    await des.region(selection_area(38, 6, 46, 10), 'lit');
    await des.door('closed', 37, 8);
    await des.door('closed', 47, 8);
    await des.door('closed', 73, 5);
    await des.door('closed', 2, 15);
    await des.mazewalk({ x: 36, y: 8, dir: 'west', stocked: 0 });
    await des.stair({ dir: 'up', coord: [42, 8] });
    await des.wallify();

    await des.object({ id: DIAMOND });
    await des.object({ class: GEM_CLASS });
    await des.object({ id: DIAMOND });
    await des.object({ class: GEM_CLASS });
    await des.object({ id: EMERALD });
    await des.object({ class: GEM_CLASS });
    await des.object({ id: EMERALD });
    await des.object({ class: GEM_CLASS });
    await des.object({ id: EMERALD });
    ...
    await des.trap();
    await des.trap();
    await des.trap({ type: 'level teleport', coord: place[1] });
    await des.trap({ type: 'level teleport', coord: place[0] });
    ...
    await des.monster({ class: S_MUMMY });
    await des.monster({ class: S_MUMMY });
    await des.monster({ id: PM_ETTIN_MUMMY });
