...
| | |
|---|---|
| **Target** | NetHack 5.0 (released May 2, 2026, the first major version bump since 3.0 in 1989; 46 years into the game's history) |
| **Open** | (announce date) |
...
| **What this measures** | The maintainability of your Phase 1 codebase. Because the diff is anchored to your Phase 1 freeze tag, the architecture and readability you freeze at the end of Phase 1 directly determine your Phase 2 ceiling. Submissions with a clean, well-factored, easy-to-modify codebase have a strong advantage; sprawling or unreadable submissions need a much larger diff to retarget. |
...
- A real upstream NetHack release after 5.0 (if one comes out in time).
- A designated post-5.0 commit on the upstream master.
- A curated set of upstream commits or backported patches.
- A feature retrofit the judges design specifically: adding a new
  monster class, a new command, or modifying an existing subsystem
  to match a slightly different specification.
