...
| 33 | Magic/fire/item damage and ignition trap effects | D:1 magic traps can select ordinary or fire-burst outcomes, damaging monsters, armor, inventory, and floor objects in strict PRNG order. The monster-fire `zap.c:destroy_items()` subset, ordinary-monster naming prerequisite, and current `apply.c:catch_lit()` subset are committed; seeds 962639 and 966115 match the live worktree consumer. Armor burning, floor burning, and the `trap.c` fire selector remain uncommitted. | `js/zap_destroy_items.js`, `js/do_name.js`, and `js/apply_catch_lit.js`; remaining `trap.c` and `zap.c` work is still temporary | partial | `undecided` | Extract the remaining `zap.c` floor and `trap.c` armor/damage owners separately, then run the grouped C13 magic/fire batch. |
