...
| 4 | `allmain.c:moveloop_core()` elapsed phase | Runs before each command dispatch when the preceding command consumed time. | `js/allmain.js` `moveloop_core()` | Owns the hero movement debit, monster allocation, random monster generation, sounds, hunger, engraving wear, and timeouts. | Three branches became two at `263540f`; `advanceFreshTurn()` now serves every turn after the first. Collapsing the last branch was tried and reverted: with one path, a turn-1 actionable monster throws `hero time effects require initialized hero_seq` instead of failing closed at the monster-action boundary. | missing | Generalize `preflightFirstFreshTurn()`'s new-game guards and reconcile the two finish gates, then remove the `(g.moves \|\| 1) === 1` branch. Closure condition 2. |
