...
13. **C13 — current D:1 `trap.c` in source-owned groups:** projectiles;
    ...
    `9573db23fbab4231b2bb0d838681ce4ef93ec5a5`. Extract the rust effect
    next. Anti-magic is
    complete at
...
- The complete `polyself.c:mbodypart()` source dispatch is committed as
  ...
  screens, and 243 cursors. The `objnam.c` armor-name prerequisite, rust
  effect, and live movement consumer remain open, so no fresh second-turn
  claim is made.
- The `objnam.c` suit, cloak, helm, and glove simple-name helpers are committed
  ...
  and 243 cursors. The rust effect and live movement consumer remain open, so
  no fresh second-turn claim is made.
