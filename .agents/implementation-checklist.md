...
| 33 | Magic/fire/item damage and ignition trap effects | D:1 magic traps can select ordinary or fire-burst outcomes, damaging monsters, armor, inventory, and floor objects in strict PRNG order. The monster-fire `zap.c:destroy_items()` subset and its naming prerequisite are committed; seeds 962639 and 966115 match the live worktree consumer. Armor burning, floor ignition, and the `trap.c` fire selector remain uncommitted. | `js/zap_destroy_items.js` and `js/do_name.js`; remaining `trap.c`, `zap.c`, and `apply.c` work is still temporary | partial | `undecided` | Extract the remaining `apply.c`, `zap.c`, and `trap.c` owners separately, then run the grouped C13 magic/fire batch. |
...
- The C13 `zap.c` fire-inventory prerequisite is committed as
  ...
  armor burning, floor burning, and item ignition remain uncommitted, so this
