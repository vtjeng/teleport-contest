...
| 10 | `monmove.c:m_move()` ordinary goal and candidates | Compute approach, hero tracking, movement flags, `m_search_items()`, `mfndpos()` candidates, and source tie-breaking for ordinary clear destinations. | `js/monmove_move.js` plus a dedicated item-search owner if needed to stay reviewable | missing | Preserve a blocking wall or door independently of earlier boulders, and port the source-ordered read-only item-selection pass so ignored objects do not stop a supported move. |
