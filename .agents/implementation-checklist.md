...
| 33 | Magic/fire/item damage and ignition trap effects | D:1 magic traps can select ordinary or fire-burst outcomes, damaging monsters, armor, inventory, and floor objects in strict PRNG order. Monster-fire inventory and floor destruction, ordinary-monster naming, and current item ignition are committed; seeds 962639 and 966115 match the live worktree consumer. Armor burning and the `trap.c` fire selector remain uncommitted. | `js/zap_destroy_items.js`, `js/do_name.js`, and `js/apply_catch_lit.js`; remaining `trap.c` armor and damage work is still temporary | partial | `undecided` | Extract the remaining `trap.c` armor/damage owner, then run the grouped C13 magic/fire batch. |
