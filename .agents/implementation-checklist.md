...
- Source review: retained the periodic-upkeep review and compared the newly
  ...
  becomes nearby.
- Focused tests: `scripts/unported-monster-actions.test.mjs` passed all 21
  tests, including live doorless movement and two-retry closed-door exclusion.
  `scripts/repeated-simple-commands.test.mjs` passed its clean-recipe oracle.
- Full suite: the checkpoint ran all 1,631 tests; all passed.
...
- Fresh differentials:
  ...
  replay-input-only segments with 18,029 matching PRNG calls and 275 matching
...
pass, and every reachable excluded path retains its atomic stop. No known
unsupported behavior remains inside this repeated-simple-command boundary.
