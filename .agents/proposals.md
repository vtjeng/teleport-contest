...
## Stop counting deferrals a sweep cannot close

**What it changes.** A deferral entry gains an optional `blockedOn` field naming
the C symbol it waits on, and `sweepCandidates()` skips a non-empty `blockedOn`
exactly as it already skips `category === 'scope'`. `.agents/selection.md:16-20`
gains the clause. The principle is the one `scripts/quality-status.mjs:1299`
already states above the `scope` test -- "Only debt a sweep can resolve counts
toward the ten" -- applied to the entries that state it in their closing
condition rather than in their category.

**Determinism.** The exclusion must never be a reading. Three mechanical
properties, in the order they matter:

- It is a stored field set once, at defer time, by the agent that read the
  source and wrote the closing condition. Counting it is the same class of test
  as `category === 'scope'`.
- It cannot be invented: validation requires the symbol to appear in
  `nethack-c/upstream/src/`, so no entry dodges a sweep by naming future work.
- It expires by itself. `AGENTS.md` requires a ported function to keep its C
  name, so once `js/` defines a function of that name the blocker has landed and
  the entry counts again. A grep decides that, not a memory -- which matters
  because three consecutive ledger records in one session had line-level claims
  that read low, and one citation rotted inside its own diff.

The dashboard prints what it excluded (`monsters holds 6 open deferrals
(4 blocked)`), because an exclusion nobody can see is how a gate quietly stops
firing.

**Scope.** The field and its validation in `scripts/quality-status.mjs`, a
`--blocked-on` flag on `defer`, the `sweepCandidates()` test, the printed
counts, and one clause in `.agents/selection.md`. `.agents/review.md` already
requires `scripts/quality-status.test.mjs` to change in the same chunk as a
threshold. Plus a one-time back-fill of the open entries, which is the reading a
sweep performs anyway.

**What prompted it.** `monsters` reached ten open non-scope entries on 10 August
2026 and gated every boundary port behind a sweep. Reading all ten found five
whose recorded closing condition names unported behavior -- the planning clone's
unfired trigger, the postmov minvis redraw, `dochug()`'s PHASE FOUR residue and
the minliquid flyer refusal all waiting on ports, and a fifth on `mattacku()`.
Those five can only be retired by the boundary ports the threshold blocks, so
the area carries a permanent floor toward its own gate. Two sweeps that day
spent six worker runs for a combined zero development screens, correctly by
their own forecasts. Excluding the five would have read six, and no sweep would
have run. The user approved the policy change in principle and asked that the
count be made reliable rather than merely smaller.

**Cost.** Small in code. The back-fill is one worker run.

**What it leaves unfixed.** Which symbol an entry waits on is still a judgement
made once, at defer time; only validity, expiry, counting and visibility become
mechanical. That is the trust model `scope` already has. It also does nothing
about the feedback that raised this gate in the first place -- a sweep's value is
that reading finds defects, and recording each find opens an entry counting
toward the same threshold that triggered the sweep. The last sweep retired five
entries and opened five.
