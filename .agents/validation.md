...
scans, scoring, browser checks, or an authorized holdout evaluation. The rules
in `AGENTS.md` for `sessions/holdout/`, the holdout whose contents are
off-limits during development, always apply. This file calls it the sealed
holdout. `.agents/workflow.md` defines a coherent implementation chunk, a
behavior slice, and a review window; this file uses those terms as defined
there.

## Routine validation

- Diagnose mismatches from the upstream C source, focused development
  recordings, and the full set of development recordings. Never diagnose from
  the sealed holdout.
...
- Redirect every checkpoint run to a log and read only its tail:
  ...
  with the test suite. Reading the checkpoint output without redirecting loses
  most of it to truncation and spends thousands of tokens on passing test names.
  The tail is about 1,200 bytes and ends with the per-check `PASS`/`FAIL`
...
the mutants that no test failed on. `--worktree` scopes uncommitted work and
`--range <base>..<head>` scopes a commit range. The instruction file for your
task states which mutation run to perform: `.claude/agents/slice-worker.md`
holds the per-slice run over uncommitted work, and `.agents/review.md`, under
"Mutation-test the reviewed lines", holds the per-window run over a frozen range
and states what a survivor proves.
...
- A recipe is the case file that `scripts/diff-fresh.mjs` replays. Recipes
  contain replay inputs only. A recipe must never contain recorded `steps` or
  reference the sealed holdout, and must not bypass the sealed-path checks that
  reject paths under `sessions/holdout/`.
...
The following five facts are common sources of wrong conclusions during
validation.
...
- **`js/terminal.js` outside the scoring workspace serializes to the empty
  ...
  measures nothing. Read `gt.toplines` (the top-line message buffer in the C tty
  struct, ported as `display.toplines`), or run inside the workspace that
...
Calculate and report a score estimate for each completed implementation chunk.
The estimate has three parts: the development-session score (shown), the holdout
score (hidden), and their sum (total). Format the estimate as:
